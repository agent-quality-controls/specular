...
docs into `speculus --help`; shrink the skill to a pointer.
...
- `main.rs`: `help` / `--help` / `-h` print `include_str!("../HELP.txt")`; bare
  no-args prints short usage + "run speculus help". Add `HELP.txt` at repo root.
...
- Add a `help` suite: golden of `speculus --help`.
...
- SKILL.md (+ codex): cut format/field/category/verifier prose now in
  `speculus --help`; keep the 3-pass extraction protocol, the coverage-map artifact,
  adjudication, and "run `speculus --help`". The workflow line "run verify before
...
2. Change 5-6 (contract + fixtures); speculus verify self-check green;
