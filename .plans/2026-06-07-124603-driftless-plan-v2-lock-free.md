...
Build `driftless`: a Rust CLI and library for deterministic spec-driven development.

`driftless` verifies that implementation matches a machine-readable JSON spec. It does not know about prose plans, Markdown descriptions, tickets, prompts, or how the spec was produced. The library starts at the JSON spec.
...
Supersedes `.plans/2026-05-14-203402-driftless-preliminary-plan.md`. Major changes from that plan:
...
`driftless` must verify repository shape and file content against a JSON contract without:
...
- All commands in V1 are read-only with respect to the repository. `driftless` writes nothing into the repository it verifies.
...
- `builtin` evidence: produced by builtin verifiers shipped and fixture-tested with `driftless`, independent of whoever authored the implementation under verification.
...
```text
https://github.com/agent-quality-controls/driftless
```
...
- `custom`: array of opaque objects. driftless validates only that each entry is an object.
...
- For typed categories, `driftless` runs the override once per block as `<command...> <spec.json> <category> <blockIndex>` from the repository root. The script reads that block and emits one JSON line per item in the block: `{"item": "...", "status": "pass" | "fail", "message": "...", "observed": ..., "expected": ..., "path": ...}`.
- For `custom`, `driftless` runs the override once as `<command...> <spec.json> custom`. The script reads the whole `requirements.custom` array. Only `status` is mandatory; all other fields are script-owned and copied into the report.
...
- resolve the input closure: spec file + every declared custom verifier file + `driftless` version
...
Driftless CLI-owned (not an `aqc-shared` crate).
...
V1 `tree` and `content` builtin verifiers are thin Driftless CLI logic over `aqc-shared` platform crates. They do not call Guardrail3 policies, connectors, or legacy family checkers (Guardrail3 vocabulary).

## Ownership boundary

| Layer | Owner |
|-------|--------|
| JSON spec, schema, semantic validation, routing, evidence, CLI | `driftless` |
...
Driftless CLI must not reimplement walk, read, or Git rules that already live in those crates' plans.
...
- spec glob matching uses driftless's own `GlobSet` (literal separator on) over `FileTree.entries`, not `FileTree::glob` (whose default lets `*` cross `/`), so spec glob semantics stay under driftless's own lint contract
...
- required path / forbidden glob checks in Driftless CLI only — no second walk semantics layer
...
When built, each backend is a per-ecosystem builtin inside `driftless` for its category and ecosystem, for example cargo dependencies, Go dependencies, or npm dependencies:

- `dependencies`: consume the ecosystem's stable machine interface — `cargo metadata`, `go list -deps -json`, package-manager graphs. No linter layer in between; a linter between `driftless` and the toolchain interface adds a vocabulary translation and a managed binary while contributing nothing.
...
`commands` and `cli` do not exist as categories: `commands` risks turning `driftless` into a generic task runner beyond the constrained verifier protocol; `cli` belongs in behavior fixtures (`fixture3`).
...
- `driftless lock <spec>`: serialize the input closure hashes to `spec.lock.json`.
- `driftless status <lock>`: compare recorded closure to current files.
- `driftless verify <lock>`: compare, refuse on mismatch, then verify.
...
Runtime walk behavior is not redefined here. Driftless CLI passes explicit options into `aqc-filetree` (see its plan: `SymlinkPolicy`, `skip_dir_names`, `.gitignore` / recovery, sorted entries). Record the chosen defaults in Driftless CLI constants when implementing builtin verifiers.
...
Git invocation and porcelain parsing are owned by `aqc-git-helpers` (`--porcelain=v1 -z`, NUL-separated records). Driftless CLI consumes helper output; it does not fork its own `git status` parser. Non-Git directories: the diagnostic field reports `NotARepository`; verification proceeds.
...
- Plan-scoped contract, done when verified -> a `driftless` requirement (spec exists for the implementation window).
...
- `driftless` executes (custom verifiers, toolchain interfaces) and parses anything, and authors nothing persistent in the repository it verifies.
...
- No shared finding/evidence crate across products; Driftless CLI owns its evidence model.
- No shared linter layer and no shared findings schema between the products.
- Driftless CLI depends on neutral `aqc-*` crates, never on guardrail3 package paths.
...
- separate CLI and config from Driftless CLI; may share `aqc-*` crates
- does not replace Driftless CLI verification; users may run both
...
7. Add the Driftless CLI evidence model with trust-grade labels.
...
13. Add fixture coverage for `driftless` itself through `fixture3`.

Do not block on Guardrail3 v2 product code — only on the neutral `aqc-*` crates.

# V1 Definition Of Done

- `driftless lint` rejects invalid JSON, JSON Schema violations, invalid typed specs, semantic validation failures, duplicate targets/items, contradictions, redundant items, vacuous specs, categories with no verifier, dead verifiers, bad custom shapes, and unknown verifier-map keys.
- `driftless verify` checks builtin `tree` and `content` requirements over `aqc-filetree`/`aqc-fs-utils` with no own walk or read stack.
- `driftless verify` runs each category's verifier (builtin or script), enforces the evidence protocol and per-item coverage, and fails loudly on protocol violations.
- Every report labels evidence `builtin`/`custom` and stamps the input closure (spec hash, verifier file hashes, `driftless` version, Git diagnostics).
