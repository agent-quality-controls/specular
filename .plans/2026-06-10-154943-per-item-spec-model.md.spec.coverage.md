...
Extraction note: this spec was not produced by three isolated sub-agent passes because the available agent tool is restricted unless the user explicitly asks for sub-agents. The accepted contract is still checked mechanically by specular after the implementation.
