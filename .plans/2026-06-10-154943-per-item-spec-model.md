...
- One verifier model: every category's verifier is declared in the `verifiers`
  ...
  evidence lines that driftless's own builtin verifiers produce.
- A `custom` category whose entries are free-form dicts; driftless never interprets
...
```json
{
  ...
  "requirements": {
    ...
    "exports": [
      { "package": "driftless", "required": ["Spec", "lint", "verify"] }
    ],
    "enumerations": [
...
- `custom` is an array of free-form objects. No schema beyond "is an object";
  driftless passes them through untouched. The example's `check`/`files` keys are
  the author's own vocabulary, not driftless's.
...
- Every verifier script — typed or custom — does its own checking and emits
  evidence lines (below). driftless does not re-judge script output; for typed
  categories it only checks bookkeeping (every item answered exactly once).

## Wire protocol (one, for all scripts)

Evidence line = the same object that appears in driftless's report:
...
- Typed categories: once per block — `<command...> <spec.json> <category>
  ...
  driftless enforces: one line per item in the block, no unknown items, no
...
3. Contract + scripts (7-8); `driftless verify` on own contract green.
