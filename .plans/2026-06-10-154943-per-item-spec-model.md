...
- One verifier model: every category's verifier is declared in the `verifiers`
  ...
  evidence lines that spec3's own builtin verifiers produce.
- A `custom` category whose entries are free-form dicts; spec3 never interprets
...
```json
{
  ...
  "requirements": {
    ...
    "exports": [
      { "package": "spec3", "required": ["Spec", "lint", "verify"] }
    ],
    "enumerations": [
...
- `custom` is an array of free-form objects. No schema beyond "is an object";
  spec3 passes them through untouched. The example's `check`/`files` keys are
  the author's own vocabulary, not spec3's.
...
- Every verifier script — typed or custom — does its own checking and emits
  evidence lines (below). spec3 does not re-judge script output; for typed
  categories it only checks bookkeeping (every item answered exactly once).

## Wire protocol (one, for all scripts)

Evidence line = the same object that appears in spec3's report:
...
- Typed categories: once per block — `<command...> <spec.json> <category>
  ...
  spec3 enforces: one line per item in the block, no unknown items, no
...
3. Contract + scripts (7-8); `spec3 verify` on own contract green.
