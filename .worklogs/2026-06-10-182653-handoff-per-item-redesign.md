...
Rust CLI + library. Verifies a repository against a JSON spec: `specular lint
<spec>`, `specular verify <spec>`. Exit codes: 0 valid/conforms, 1 nonconform,
...
- Gates: `cargo check/clippy/fmt` clean; `fixture3 check --all` matched (two
  ...
  (`.plans/2026-06-07-124603-specular-plan-v2-lock-free.md.spec.json`) exit 0.
- `specular --help` prints `HELP.txt` (committed, embedded via include_str!).
...
- `custom` category: free-form dict entries specular never interprets; its
...
2. `.plans/2026-06-07-124603-specular-plan-v2-lock-free.md` (product plan; some
