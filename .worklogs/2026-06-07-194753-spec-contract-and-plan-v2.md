# Spec3 plan v2 (lock-free) + build contract
...
- aqc backends attach in V1 (crates landed in aqc-shared 2026-06-07); spec glob matching uses spec3's own GlobSet (literal separator on), not `FileTree::glob`.
...
- `.plans/2026-06-07-124603-spec3-plan-v2-lock-free.md` — the plan (single source of truth)
...
- fixture3 behavior suite per the fixture development guide: layered fixtures (R00 clean golden -> R10 breakage layers), `fixture3.yaml` at repo root, replay = run `spec3` against fixture trees, one JSON record per command.
