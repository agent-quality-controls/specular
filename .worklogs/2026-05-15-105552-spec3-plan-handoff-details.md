Summary
- Updated the `spec3` preliminary plan with missing handoff details for a fresh implementation agent.
...
- Required `spec3 lock` to fail on dirty worktrees by default so uncommitted spec/verifier changes cannot become hidden contract inputs.
- Required `spec3 verify` to fail when locked plan, spec, or verifier files are dirty, while allowing dirty implementation files in V1.
