...
Added the fixture3 behavior suite per the guardrails fixture development guide: two suites (lint, verify), 11 layered fixtures replaying the real spec3 binary, goldens approved, `fixture3 check --all` matched. Both gates green: build contract (spec-verify 9/9, exit 0) and behavior (fixture3 matched).

## Decisions made

- Layer ladder per the guide, grouping unit = command (spec3 has no policies/families):
