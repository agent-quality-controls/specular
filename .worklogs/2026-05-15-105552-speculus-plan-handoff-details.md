Summary
- Updated the `speculus` preliminary plan with missing handoff details for a fresh implementation agent.
...
- Required `speculus lock` to fail on dirty worktrees by default so uncommitted spec/verifier changes cannot become hidden contract inputs.
- Required `speculus verify` to fail when locked plan, spec, or verifier files are dirty, while allowing dirty implementation files in V1.
